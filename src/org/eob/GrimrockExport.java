...
*/
    ...
    // Exports wall type definitions
    void exportGrimrockWallTypes(String filename, int minLevel, int maxLevel) {
    try {
        // Let's generate a list of all unique types that are there
        HashMap<String, Wall> objects = new HashMap<String, Wall>();

        // Let's export objects for the first level only
        for (int level = minLevel; level <= maxLevel; level++) {
            for (int i = 0; i <= 67; i++) {
                 Wall x = Wall.getById(i, level);
                 if (x.elementName.isEmpty())
                     continue;
                 if (objects.containsKey(x.elementName)) // we have that type already, let's skip it
                     continue;
        ...
        file.println(String.format("-- This file has been generated by EobConverter v%s", EobConverter.CONVERTER_VERSION));
        file.println("");

        // Now let's generate definitions for all of them
        for (Map.Entry<String, Wall> entry : objects.entrySet()) {
            Wall x= entry.getValue();
            // System.out.println(entry.getKey() + "/" + entry.getValue());
            file.println("cloneObject {");
            file.println("    name = \"" + x.elementName + "\",");
            file.println("    baseObject = \"" + x.elementType + "\",");
            file.println("}");
            file.println();
        }

        file.close();
        System.out.println("Wrote " + objects.size() + " element definitions to " + filename + " file.");
        } catch (IOException e) {
            System.out.println("Problem writing wall type definitions: " + e.getMessage());
        }
    }
